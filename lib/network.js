...
async function checkPPPoEExpirations() {
  try {
    ...
    const now = new Date();
    // console.log(`[PPPoE-Server] Checking expirations for ${users.length} users at ${now.toISOString()}`);
    ...
    for (const user of users) {
      ...
      if (expDate < now) {
        if (user.expiration_action === 'disable') {
           ...
           // Auto-kick (Kill active session) so they are forced to reconnect and hit the firewall rules immediately
           // Check if they are currently connected but NOT yet in the redirect map (newly expired)
           const sessions = await getPPPoESessions();
           const session = sessions.find(s => s.username === user.username);
           
           // Only kick if they have an active session AND we haven't already processed them (not in redirect map yet)
           // OR if they are connected but somehow bypassed the redirect (reconnected quickly before rule applied)
           if (session) {
