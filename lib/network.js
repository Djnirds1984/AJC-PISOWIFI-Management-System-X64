...
async function startPPPoEServer(config) {
  ...
  try {
    ...
    // 6. Start pppoe-server daemon without IP pool (will be handled by profiles)
    const serviceNameArg = service_name ? `-S "${service_name}" -C "${service_name}"` : '';
    // We keep nohup and background execution for app stability
    // Remove IP pool range - this will be handled by individual profiles
    const cmd = `nohup pppoe-server -I ${targetIface} -L ${local_ip} ${serviceNameArg} -O ${optionsPath} >> /var/log/pppoe-server.log 2>&1 &`;
