...
async function startPPPoEServer(config) {
  ...
  try {
    ...
    // Force PPPoE server to use our IP pool range 100.15.0.100-100.15.0.254
    const cmd = `nohup pppoe-server -I ${targetIface} -L ${local_ip} -p 100.15.0.100,100.15.0.254 ${serviceNameArg} -O ${optionsPath} >> /var/log/pppoe-server.log 2>&1 &`;
