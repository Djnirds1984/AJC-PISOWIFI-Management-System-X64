    ...
    for (const user of expiredUsers) {
      ...
      if (session && session.ip) {
        ...
        if (!currentRedirectUrl || currentRedirectUrl !== user.redirect_url) {
           ...
           // 1. Allow DNS (Critical for redirection to work)
           ...
           // Also allow Input to local DNS if running on router
           await execPromise(`iptables -I INPUT 1 -s ${session.ip} -p udp --dport 53 -j ACCEPT`).catch(()=>{});

           // 2. Redirect HTTP (80) to 8081 (Payment Portal)
           await execPromise(`iptables -t nat -I PREROUTING -s ${session.ip} -p tcp --dport 80 -j REDIRECT --to-port 8081`).catch(()=>{});
           
           // 3. Block all Internet Access (FORWARD chain) - Insert at top (will push previous ACCEPTs down if using -I 1? No, -I 1 pushes existing 1 to 2.)
           // So if we insert DROP at 1, it becomes 1. The DNS accepts become 2 and 3.
           // This is WRONG. DROP at 1 means it hits DROP first.
           
           // Correct Order:
           // 1. ACCEPT DNS
           // 2. DROP ALL
           
           // Strategy: Insert DROP first (becomes 1). Then Insert ACCEPT DNS at 1 (becomes 1, DROP becomes 2).
           
           await execPromise(`iptables -I FORWARD 1 -s ${session.ip} -j DROP`).catch(()=>{});
           // Now Insert DNS ACCEPT at 1, pushing DROP to 2
           await execPromise(`iptables -I FORWARD 1 -s ${session.ip} -p udp --dport 53 -j ACCEPT`).catch(()=>{});
           await execPromise(`iptables -I FORWARD 1 -s ${session.ip} -p tcp --dport 53 -j ACCEPT`).catch(()=>{});
        }
      }
  ...
  try {
    ...
    for (const user of users) {
      ...
      if (expDate < now) {
        if (user.expiration_action === 'disable') {
           ...
           if (session) {
             ...
             if (!alreadyRedirected) {
                // Logic to kick ONCE to force reconnection/awareness
                if (kickedUsernames.has(user.username)) {
                    console.log(`[PPPoE] User ${user.username} reconnected. Applying redirect rules.`);
                    // Fall through to syncPPPoERedirects to apply rules
                } else {
                    console.log(`[PPPoE-Server] Auto-kicking newly expired user ${user.username} to enforce redirect rules.`);
                    kickedUsernames.add(user.username);
                    await killPPPoESession(session.interface);
                }
             }
           }
        ...
        if (kickedUsernames.has(user.username)) {
            kickedUsernames.delete(user.username);
        }
      }
    }
