    ...
    for (const pppIface of pppInterfaces) {
      ...
      try {
        ...
        if (fs.existsSync(sessionFile)) {
           ...
           // Method 2: Fallback to grep logs
           const grepCmd = `grep "${ifname}.*user" /var/log/pppd.log | tail -n 1`;
           const { stdout: logMatch } = await execPromise(grepCmd).catch(() => ({ stdout: '' }));
           
           if (logMatch) {
               const match = logMatch.match(/user\s+["']([^"']+)["']/) || logMatch.match(/user\s+([^ ]+)/);
               if (match) username = match[1];
           }
        }

        // Fallback: Check /var/run/ppp/ if specific files exist that might help (optional)
      } catch (e) {}
