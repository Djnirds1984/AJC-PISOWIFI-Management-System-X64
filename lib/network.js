  ...
  try {
    ...
    for (const user of expiredUsers) {
      ...
      if (session && session.ip) {
        ...
        if (!pppoeRedirects.has(session.ip)) {
           console.log(`[PPPoE-Server] Applying redirect for ${user.username} (${session.ip})`);
           pppoeRedirects.set(session.ip, user.redirect_url);
           // Apply iptables rule to intercept HTTP
           await execPromise(`iptables -t nat -I PREROUTING -s ${session.ip} -p tcp --dport 80 -j REDIRECT --to-port 80`).catch(()=>{});
        } else {
           // Update URL if changed
           if (pppoeRedirects.get(session.ip) !== user.redirect_url) {
              pppoeRedirects.set(session.ip, user.redirect_url);
           }
        }
      }
    ...
    for (const [ip, url] of pppoeRedirects.entries()) {
       if (!activeExpiredIPs.has(ip)) {
          console.log(`[PPPoE-Server] Removing redirect for ${ip}`);
          pppoeRedirects.delete(ip);
          // Remove iptables rule
          await execPromise(`iptables -t nat -D PREROUTING -s ${ip} -p tcp --dport 80 -j REDIRECT --to-port 80`).catch(()=>{});
       }
    }
  } catch (e) {
