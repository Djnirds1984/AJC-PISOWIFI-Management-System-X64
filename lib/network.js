...
async function getPPPoESessions() {
  try {
    ...
    for (const pppIface of pppInterfaces) {
      ...
      // For Point-to-Point interfaces (PPPoE), the client IP is the 'peer' address.
      // 'local' is the server's address on that interface.
      const ip = pppIface.addr_info?.[0]?.peer || pppIface.addr_info?.[0]?.local || 'N/A';
      
      // Try to get username from pppd logs using grep for efficiency and robustness
      let username = 'Unknown';
      try {
        // Method 1: Check session logger file (most reliable)
        const sessionFile = `/var/run/ajc-pppoe/${ifname}`;
        if (fs.existsSync(sessionFile)) {
           username = fs.readFileSync(sessionFile, 'utf8').trim();
        } else {
           // Method 2: Fallback to grep logs - EXPANDED SEARCH
           // Search both pppd.log and pppoe-server.log
           // Look for "user=" pattern or "Connect:" lines which might be nearby
           const grepCmd = `grep -h "${ifname}" /var/log/pppd.log /var/log/pppoe-server.log | grep "user" | tail -n 1`;
           const { stdout: logMatch } = await execPromise(grepCmd).catch(() => ({ stdout: '' }));
           
           if (logMatch) {
               const match = logMatch.match(/user\s+["']?([^"'\s]+)["']?/) || logMatch.match(/user="([^"]+)"/);
               if (match) username = match[1];
           }
           
           // Method 3: Check /var/run/ppp/ folder for PID files which might contain info
           // Sometimes pppd stores peer name in internal structures we can't easily read,
           // but we can try to infer from process arguments if available
           if (username === 'Unknown') {
             const pidFile = `/var/run/${ifname}.pid`;
             if (fs.existsSync(pidFile)) {
                const pid = fs.readFileSync(pidFile, 'utf8').trim();
                // Check process arguments for 'user' or 'name'
                const { stdout: psOut } = await execPromise(`ps -p ${pid} -o args=`).catch(() => ({ stdout: '' }));
                const userArgMatch = psOut.match(/user\s+([^ ]+)/);
                if (userArgMatch) username = userArgMatch[1];
             }
           }
        }
      } catch (e) {}
      
      // Fallback: Use radutmp or who/last if available (rarely populated by simple pppoe-server)
    ...
    // console.log(`[PPPoE-Debug] Found ${sessions.length} sessions:`, sessions.map(s => `${s.username}=${s.ip}`).join(', '));
      ...
      if (expDate < now) {
         ...
         // } else if (user.expiration_action === 'redirect') {
           ...
           if (session) {
             ...
             // Debug log why session not found
