...
async function checkPPPoEExpirations() {
  try {
    const users = await db.all("SELECT * FROM pppoe_users WHERE enabled = 1 AND expiration_date IS NOT NULL AND expiration_date != ''");
    const now = new Date();
    let changed = false;
    ...
    for (const user of users) {
      ...
      if (expDate < now) {
        console.log(`[PPPoE-Server] User ${user.username} expired at ${user.expiration_date}. Action: ${user.expiration_action}`);
        
        // Disable user (for both 'disable' and 'redirect' actions)
        // Note: 'redirect' currently falls back to disable as we don't have a captive portal redirect mechanism for PPPoE yet.
        await db.run('UPDATE pppoe_users SET enabled = 0 WHERE id = ?', [user.id]);
        changed = true;
