...
async function addPPPoEUser(username, password, billing_profile_id = null, expiration_date = null, expiration_action = 'disable', redirect_url = null) {
  console.log(`[PPPoE-Server] Adding user: ${username}`);
  
  try {
    // 1. Add to database
    await db.pppoeRun(
      'INSERT INTO pppoe_users (username, password, enabled, billing_profile_id, expiration_date, expiration_action, redirect_url) VALUES (?, ?, 1, ?, ?, ?, ?)',
      [username, password, billing_profile_id, expiration_date, expiration_action, redirect_url]
    );
...
async function updatePPPoEUser(userId, updates) {
  try {
    const { username, password, enabled, billing_profile_id, expiration_date, expiration_action, redirect_url } = updates;
