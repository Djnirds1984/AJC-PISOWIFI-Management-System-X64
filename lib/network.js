...
async function getPPPoESessions() {
  try {
    ...
    for (const pppIface of pppInterfaces) {
      ...
      try {
        // Look for "user 'username'" or "user="username"" associated with this interface in the logs
        // We look for the most recent entry for this interface
        const grepCmd = `grep "${ifname}.*user" /var/log/pppd.log | tail -n 1`;
        const { stdout: logMatch } = await execPromise(grepCmd).catch(() => ({ stdout: '' }));
        
        if (logMatch) {
            // Match: user 'username' OR user "username"
    ...
    for (const user of expiredUsers) {
      ...
      if (session && session.ip) {
        ...
        if (!currentRedirectUrl || currentRedirectUrl !== user.redirect_url) {
           ...
           // 2. Block all Internet Access (FORWARD chain) - Insert at top
