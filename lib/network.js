...
// Track PPPoE Redirect States
const pppoeRedirects = new Map();
// Track Kicked Users to prevent infinite kick loops
const kickedUsernames = new Set();
// Track processed expired users to prevent redundant logging
const processedExpiredUsers = new Set();
...
async function initFirewall() {
  ...
  try {
    ...
    // Clear PPPoE Redirect Cache so rules are re-applied
    pppoeRedirects.clear();
    ...
    // REMOVED: This rule was bypassing the payment redirect logic.
    // PPPoE users should be subject to specific redirects if expired.
    ...
    // 6. Initialize Anti-VPN if enabled
    await initAntiVPN();
  } catch (e) {
...
async function setupHotspot(config, skipRestart = false) {
  ...
  try {
    ...
    await execPromise(`iptables -t mangle -D POSTROUTING -o ${interface} -j TTL --ttl-set 1`).catch(() => {});
    await execPromise(`iptables -t mangle -A POSTROUTING -o ${interface} -j TTL --ttl-set 1`).catch(() => {});
...
async function removeHotspot(interface, skipRestart = false) {
  try {
    ...
    await execPromise(`iptables -t mangle -D POSTROUTING -o ${targetInterface} -j TTL --ttl-set 1`).catch(() => {});
...
async function restoreNetworkConfig() {
  ...
  try {
    ...
    const pppoeServer = await db.pppoeGet('SELECT * FROM pppoe_server WHERE enabled = 1');
    if (pppoeServer) {
      try {
  ...
  try {
    ...
    if (wanName) {
      ...
      const vlanConfigs = [
        ...
        let name = `${wanName}.${config.id}`;
        // Linux interface name limit is 15 chars. If too long, use short naming convention.
        if (name.length > 15) {
          name = `vlan${config.id}`;
        }
        return { ...config, name };
...
async function setupPPPoELogger() {
  try {
    const ipUpPath = '/etc/ppp/ip-up.d/00-ajc-logger';
    const ipDownPath = '/etc/ppp/ip-down.d/00-ajc-logger';
    const logDir = '/var/run/ajc-pppoe';
    ...
    if (!fs.existsSync('/etc/ppp/ip-up.d')) fs.mkdirSync('/etc/ppp/ip-up.d', { recursive: true });
    if (!fs.existsSync('/etc/ppp/ip-down.d')) fs.mkdirSync('/etc/ppp/ip-down.d', { recursive: true });

    const ipUpContent = `#!/bin/sh
# AJC PPPoE Session Logger & DB Sync
IFNAME=$1
PEERNAME=$PEERNAME
IP=$IPREMOTE
MAC=$MACREMOTE

# 1. File Logging (Legacy)
mkdir -p ${logDir}
if [ -n "$PEERNAME" ]; then
  echo "$PEERNAME" > ${logDir}/$IFNAME
else
  echo "Unknown" > ${logDir}/$IFNAME
fi
chmod 644 ${logDir}/$IFNAME

# 2. Database Sync (Robust)
# Use node script to insert into pppoe_onlines
# Arguments: up <ifname> <peername> <ip> <mac>
node /opt/ajc-pisowifi/scripts/pppoe-hook.js up "$IFNAME" "$PEERNAME" "$IP" "$MAC" >> /var/log/pppoe-hook.log 2>&1 &
`;

    const ipDownContent = `#!/bin/sh
# AJC PPPoE Session Logger & DB Sync
IFNAME=$1
...
# 2. Database Sync
# Arguments: down <ifname>
node /opt/ajc-pisowifi/scripts/pppoe-hook.js down "$IFNAME" >> /var/log/pppoe-hook.log 2>&1 &
`;

    fs.writeFileSync(ipUpPath, ipUpContent);
    fs.writeFileSync(ipDownPath, ipDownContent);

    await execPromise(`chmod +x ${ipUpPath}`);
    await execPromise(`chmod +x ${ipDownPath}`);

    console.log('[PPPoE-Server] Session logger scripts installed.');
  } catch (e) {
    console.error('[PPPoE-Server] Failed to setup logger:', e.message);
  }
}

async function startPPPoEServer(config, profile = null) {
  let { interface: iface, local_ip, dns1 = '8.8.8.8', dns2 = '8.8.4.4', service_name = '' } = config;
  
  // Use profile IP pool if provided
  let ipPoolStart = '100.15.0.100';
  let ipPoolEnd = '100.15.0.254';
  if (profile && profile.ip_pool_start && profile.ip_pool_end) {
    ipPoolStart = profile.ip_pool_start;
    ipPoolEnd = profile.ip_pool_end;
    console.log(`[PPPoE-Server] Using profile IP pool: ${ipPoolStart}-${ipPoolEnd}`);
  }
  ...
  try {
    ...
    const optionsContent = [
      ...
      `mtu 1492`,
      `idle 0`,
      `# Force server IP assignment - reject client IP requests`,
      `noip`,
      `ipcp-accept-local`,
      `ipcp-accept-remote`,
      `# Set IP range for PPPoE server - force 100.15.0.x pool`,
      `${local_ip}:100.15.0.100-100.15.0.254`,
      `debug`,
      `dump`,
    ...
    // 5.1 Create main PPP options file to force server IP assignment
    const mainOptionsPath = '/etc/ppp/options';
    const mainOptionsContent = [
      `# AJC PisoWiFi Main PPP Options`,
      `# Force server IP assignment - reject client IP requests`,
      `-ipcp-accept-remote`,
      `-ipcp-accept-local`,
      `# Set local IP to force server control`,
      `${local_ip}:`,
      `lock`,
      `local`,
      `auth`,
    ...
    fs.writeFileSync(mainOptionsPath, mainOptionsContent);
    
    // 5.2 Create IP pool file for pppoe-server
    const poolPath = '/etc/ppp/pppoe-ip-pool';
    const poolContent = [
      `# AJC PisoWiFi IP Pool`,
      `# IP range for PPPoE clients`,
      `${ipPoolStart}-${ipPoolEnd}`
    ].join('\n');
    
    fs.writeFileSync(poolPath, poolContent);
    ...
    // 5.0 Install Logger Scripts
    await setupPPPoELogger();
    ...
    // 6. Start pppoe-server daemon with IP pool range
    const serviceNameArg = service_name ? `-S "${service_name}" -C "${service_name}"` : '';
    // Force PPPoE server to use our IP pool range 100.15.0.100-100.15.0.254 (using pool file)
    const cmd = `nohup pppoe-server -I ${targetIface} -L ${local_ip} -p ${poolPath} ${serviceNameArg} -O ${optionsPath} >> /var/log/pppoe-server.log 2>&1 &`;
    ...
    if (isRunning) {
      ...
      // 9. Save to database (without IP pool - will be handled by profiles)
      await db.pppoeRun(
        'INSERT OR REPLACE INTO pppoe_server (interface, local_ip, dns1, dns2, service_name, enabled) VALUES (?, ?, ?, ?, ?, 1)',
        [targetIface, local_ip, dns1, dns2, service_name]
      );
...
async function getPPPoEServerStatus() {
  try {
    const config = await db.pppoeGet('SELECT * FROM pppoe_server LIMIT 1');
...
async function getPPPoESessions() {
  try {
    // HYBRID APPROACH: DB + System Interfaces
    // 1. Get confirmed sessions from DB (Reliable Username mapping)
    const dbSessions = await db.pppoeAll("SELECT * FROM pppoe_onlines");
    
    // 2. Get active interfaces from System (Real-time Status)
    ...
    for (const pppIface of pppInterfaces) {
      ...
      const ip = pppIface.addr_info?.[0]?.peer || pppIface.addr_info?.[0]?.local || 'N/A';
      
      // Match with DB record
      const dbRecord = dbSessions.find(s => s.interface === ifname);
      ...
      if (dbRecord) {
         ...
         // Fallback to legacy detection methods if DB record missing
         try {
           const sessionFile = `/var/run/ajc-pppoe/${ifname}`;
           if (fs.existsSync(sessionFile)) {
              username = fs.readFileSync(sessionFile, 'utf8').trim();
           }
         } catch(e) {}
      }
...
async function syncPPPoESecrets() {
  ...
  try {
    const users = await db.pppoeAll('SELECT username, password FROM pppoe_users WHERE enabled = 1');
...
async function addPPPoEUser(username, password, billing_profile_id = null, expiration_date = null, expiration_action = 'disable', expiration_profile_id = null, redirect_url = null) {
  console.log(`[PPPoE-Server] Adding user: ${username}`);
  
  try {
    // 1. Add to database
    await db.pppoeRun(
      'INSERT INTO pppoe_users (username, password, enabled, billing_profile_id, expiration_date, expiration_action, expiration_profile_id, redirect_url) VALUES (?, ?, 1, ?, ?, ?, ?, ?)',
      [username, password, billing_profile_id, expiration_date, expiration_action, expiration_profile_id, redirect_url]
    );
...
async function deletePPPoEUser(userId) {
  ...
  try {
    ...
    await db.pppoeRun('DELETE FROM pppoe_users WHERE id = ?', [userId]);
...
async function getPPPoEUsers() {
  try {
    const users = await db.pppoeAll('SELECT * FROM pppoe_users ORDER BY created_at DESC');
    return users;
  } catch (e) {
...
async function updatePPPoEUser(userId, updates) {
  try {
    const { username, password, enabled, billing_profile_id, expiration_date, expiration_action, expiration_profile_id, redirect_url } = updates;
    
    // Get current user
    const currentUser = await db.pppoeGet('SELECT * FROM pppoe_users WHERE id = ?', [userId]);
    ...
    if (expiration_action !== undefined) { fields.push('expiration_action = ?'); values.push(expiration_action); }
    if (expiration_profile_id !== undefined) { fields.push('expiration_profile_id = ?'); values.push(expiration_profile_id); }
    if (redirect_url !== undefined) { fields.push('redirect_url = ?'); values.push(redirect_url); }
    
    if (fields.length > 0) {
      values.push(userId);
      await db.pppoeRun(`UPDATE pppoe_users SET ${fields.join(', ')} WHERE id = ?`, values);
    }
...
async function killPPPoESession(ifname) {
  try {
    const pidPath = `/var/run/${ifname}.pid`;
    if (fs.existsSync(pidPath)) {
      const pid = fs.readFileSync(pidPath, 'utf8').trim();
      await execPromise(`kill ${pid}`);
    } else {
      // Try to find PID via ps if pid file missing
      const { stdout } = await execPromise(`ps aux | grep "pppd" | grep "${ifname}" | grep -v grep | awk '{print $2}'`);
      const pid = stdout.trim().split('\n')[0];
      if (pid) await execPromise(`kill ${pid}`);
    }
  } catch (e) {
    console.error(`[PPPoE-Server] Failed to kill session ${ifname}:`, e.message);
  }
}

async function syncPPPoERedirects(expiredUsers) {
  try {
    const sessions = await getPPPoESessions();
    const activeExpiredIPs = new Set();
    
    for (const user of expiredUsers) {
      const session = sessions.find(s => s.username === user.username);
      if (session && session.ip) {
        activeExpiredIPs.add(session.ip);
        
        // Add to map if not present OR if IP changed (reconnected)
        const currentRedirectUrl = pppoeRedirects.get(session.ip);
        
        if (!currentRedirectUrl || currentRedirectUrl !== user.redirect_url) {
           console.log(`[PPPoE-Server] Applying non-payment redirect/block for ${user.username} (${session.ip})`);
           pppoeRedirects.set(session.ip, user.redirect_url);
           
           // Clean existing rules for this IP just in case to avoid duplicates
           await execPromise(`iptables -t nat -D PREROUTING -s ${session.ip} -p tcp --dport 80 -j REDIRECT --to-port 8081`).catch(()=>{});
           await execPromise(`iptables -D FORWARD -s ${session.ip} -j DROP`).catch(()=>{});
           await execPromise(`iptables -D FORWARD -s ${session.ip} -p udp --dport 53 -j ACCEPT`).catch(()=>{});
           await execPromise(`iptables -D FORWARD -s ${session.ip} -p tcp --dport 53 -j ACCEPT`).catch(()=>{});

           // 1. Redirect HTTP (80) to 8081 (Payment Portal)
           await execPromise(`iptables -t nat -I PREROUTING -s ${session.ip} -p tcp --dport 80 -j REDIRECT --to-port 8081`).catch(()=>{});
           
           // 2. Block all Internet Access (FORWARD chain) - Insert DROP at 1
           await execPromise(`iptables -I FORWARD 1 -s ${session.ip} -j DROP`).catch(()=>{});

           // 3. Allow DNS (Critical for redirection to work) - Insert ACCEPT at 1 (pushes DROP to 2)
           await execPromise(`iptables -I FORWARD 1 -s ${session.ip} -p udp --dport 53 -j ACCEPT`).catch(()=>{});
           await execPromise(`iptables -I FORWARD 1 -s ${session.ip} -p tcp --dport 53 -j ACCEPT`).catch(()=>{});
           
           // 4. Also allow Input to local DNS/Portal if needed
           await execPromise(`iptables -I INPUT 1 -s ${session.ip} -p udp --dport 53 -j ACCEPT`).catch(()=>{});
           await execPromise(`iptables -I INPUT 1 -s ${session.ip} -p tcp --dport 8081 -j ACCEPT`).catch(()=>{});
        }
        }
      }
    
    // Cleanup IPs that are no longer expired or no longer connected
    for (const [ip, url] of pppoeRedirects.entries()) {
       if (!activeExpiredIPs.has(ip)) {
          console.log(`[PPPoE-Server] Removing redirect/block for ${ip}`);
          pppoeRedirects.delete(ip);
          
          // Also remove from processed expired users set
          // Find username for this IP
          const session = sessions.find(s => s.ip === ip);
          if (session) {
            processedExpiredUsers.delete(session.username);
          }
          
          // Remove iptables rules
          await execPromise(`iptables -t nat -D PREROUTING -s ${ip} -p tcp --dport 80 -j REDIRECT --to-port 8081`).catch(()=>{});
          await execPromise(`iptables -D FORWARD -s ${ip} -j DROP`).catch(()=>{});
          await execPromise(`iptables -D FORWARD -s ${ip} -p udp --dport 53 -j ACCEPT`).catch(()=>{});
          await execPromise(`iptables -D FORWARD -s ${ip} -p tcp --dport 53 -j ACCEPT`).catch(()=>{});
          await execPromise(`iptables -D INPUT -s ${ip} -p udp --dport 53 -j ACCEPT`).catch(()=>{});
       }
    }
  } catch (e) {
    console.error('[PPPoE-Server] Sync Redirects Error:', e.message);
  }
}

// Helper to parse dates robustly
function parsePPPoEDate(dateStr) {
  ...
  // Try parsing MM/DD/YYYY hh:mm A format (e.g. 02/01/2026 10:05 AM)
  try {
    ...
    if (parts.length >= 2) {
      ...
      if (dateParts.length === 3) {
        ...
        d = new Date(
          ...
          parseInt(dateParts[0]) - 1, // MM (0-based)
...
async function checkPPPoEExpirations() {
  try {
    const users = await db.pppoeAll("SELECT * FROM pppoe_users WHERE enabled = 1 AND expiration_date IS NOT NULL AND expiration_date != ''");
    const now = new Date();
    // Get active sessions once for efficiency
    const sessions = await getPPPoESessions();
    
    // Log current server time for debugging timezone issues
    console.log(`[PPPoE-Server] Expiration Check at ${now.toString()} (TZ: ${process.env.TZ || 'System'})`);

    let changed = false;
    const expiredRedirectUsers = [];

    for (const user of users) {
      const expDate = parsePPPoEDate(user.expiration_date);
      
      if (!expDate || isNaN(expDate.getTime())) {
        console.log(`[PPPoE] Invalid expiration date for ${user.username}: ${user.expiration_date}`);
        continue;
      }
      
      // Debug logging for individual users - only log if user has active session or hasn't been processed
      const session = sessions.find(s => s.username === user.username);
      const isProcessed = processedExpiredUsers.has(user.username);
      
      if (session || !isProcessed) {
        console.log(`[PPPoE] Checking ${user.username}: Exp=${expDate.toString()} (${user.expiration_date}) vs Now=${now.toString()}`);
      }

      if (expDate < now) {
         // Override 'disable' action to 'redirect' to keep user connected for payment
          // if (user.expiration_action === 'disable') {
          //    console.log(`[PPPoE-Server] User ${user.username} expired (disable).`);
          //    await db.pppoeRun('UPDATE pppoe_users SET enabled = 0 WHERE id = ?', [user.id]);
          //    changed = true;
          //    
          //    // Kill active session
          //    const session = sessions.find(s => s.username === user.username);
          //    if (session) {
          //      console.log(`[PPPoE-Server] Killing expired session for ${user.username} on ${session.interface}`);
          //      // DO NOT KILL SESSION FOR REDIRECT
          //      // await killPPPoESession(session.interface);
          //    }
          // } else if (user.expiration_action === 'redirect') {
           // console.log(`[PPPoE-Server] User ${user.username} expired (redirect).`);
           expiredRedirectUsers.push(user);
           
           if (session) {
             const alreadyRedirected = pppoeRedirects.has(session.ip);
             
             if (!alreadyRedirected) {
                console.log(`[PPPoE-Server] User ${user.username} expired. Applying redirect rules immediately.`);
             }
             // Mark as processed since user has active session
             processedExpiredUsers.add(user.username);
           } else {
             // Only log session not found message once per user
             if (!isProcessed) {
               console.log(`[PPPoE-Debug] User ${user.username} expired but no active session found. Available sessions: ${sessions.map(s => s.username).join(',')}`);
               processedExpiredUsers.add(user.username);
             }
           }
        // }
      } else {
        // User NOT expired (or renewed) - remove from processed set if present
        if (isProcessed) {
          processedExpiredUsers.delete(user.username);
        }
      }
    }

    // Handle Redirects
    await syncPPPoERedirects(expiredRedirectUsers);

    if (changed) {
      await syncPPPoESecrets();
    }
  } catch (e) {
    console.error('[PPPoE-Server] Expiration check error:', e.message);
  }
}

// Cleanup processed expired users to prevent memory growth
function cleanupProcessedExpiredUsers() {
  const beforeSize = processedExpiredUsers.size;
  
  // Remove users who haven't had active sessions for a while
  // This prevents memory growth but keeps recently processed users
  if (processedExpiredUsers.size > 100) {
    // Convert to array and keep only the last 50 entries
    const users = Array.from(processedExpiredUsers);
    processedExpiredUsers.clear();
    users.slice(-50).forEach(user => processedExpiredUsers.add(user));
  }
  
  if (beforeSize !== processedExpiredUsers.size) {
    console.log(`[PPPoE-Server] Cleaned up processed expired users: ${beforeSize} -> ${processedExpiredUsers.size}`);
  }
}


async function configureInterface(config, index) {
  const { interface: iface, type, ip, netmask, gateway, username, password, mac } = config;
  console.log(`[NET] Configuring ${iface} as ${type}...`);
  
  try {
    // Handle MAC Address Cloning (Super Router Feature)
    if (mac && mac.length === 17) {
        console.log(`[NET] Cloning MAC address ${mac} to ${iface}`);
        try {
            await execPromise(`ip link set dev ${iface} down`);
            await execPromise(`ip link set dev ${iface} address ${mac}`);
            await execPromise(`ip link set dev ${iface} up`);
        } catch (e) {
            console.error(`[NET] Failed to clone MAC: ${e.message}`);
            // Attempt to bring it back up if it failed
            await execPromise(`ip link set dev ${iface} up`).catch(() => {});
        }
    } else {
    ...
    if (type === 'dhcp') {
       ...
       await execPromise(`pkill -f "dhclient -v ${iface}"`).catch(() => {});
       await execPromise(`dhclient -r ${iface}`).catch(() => {});
       
       // Start new dhclient
       await execPromise(`dhclient -v ${iface}`);
       return iface;
    } 
    else if (type === 'static') {
       if (!ip || !netmask) throw new Error('Missing IP/Netmask for static config');
       await execPromise(`ip addr flush dev ${iface}`);
       await execPromise(`ip addr add ${ip}/${netmask} dev ${iface}`);
       return iface;
    } 
    else if (type === 'pppoe') {
       // PPPoE Client
       // We use a unit number to predict the interface name
       // Start from unit 10 to avoid conflict with server (usually 0-9?) or just use high range
       const unit = 10 + index; 
       const pppName = `ppp${unit}`;
       
       // Create peer file
       const peerName = `wan_${iface}`;
       const peerFile = `/etc/ppp/peers/${peerName}`;
       
       // Ensure directory exists
       if (!fs.existsSync('/etc/ppp/peers')) {
         fs.mkdirSync('/etc/ppp/peers', { recursive: true });
       }
       ...
       const content = [
         ...
         `holdoff 5`,
         `noauth`,
         `nodefaultroute`, // We manage routing manually for MultiWAN
         `usepeerdns`
       ].join('\n');
       ...
       // Stop existing
       await execPromise(`pkill -f "pppd call ${peerName}"`).catch(() => {});
       
       // Start
       await execPromise(`pppd call ${peerName}`);
       
       // Wait for interface to appear (max 5 seconds)
       let attempts = 0;
       while (attempts < 10) {
         try {
            await execPromise(`ip link show ${pppName}`);
            console.log(`[NET] PPPoE interface ${pppName} is up.`);
            break;
         } catch(e) {
    ...
    console.error(`[NET] Config Interface Error (${iface}):`, e.message);
    // Return original interface as fallback
    return iface;
  }
}
...
module.exports = { 
  ...
  checkTcRulesExist,
  applyGamingPriority,
  getPPPoERedirect: (ip) => pppoeRedirects.get(ip),
  toggleAntiVPN
};

async function applyAntiVPNRules() {
  console.log('[NET] Applying Anti-VPN Rules...');
  try {
    // Block PPTP (TCP 1723, Protocol 47 GRE)
    await execPromise('iptables -I FORWARD -p tcp --dport 1723 -j DROP').catch(()=>{});
    await execPromise('iptables -I FORWARD -p 47 -j DROP').catch(()=>{});

    // Block L2TP/IPsec (UDP 500, 4500, 1701)
    await execPromise('iptables -I FORWARD -p udp --dport 500 -j DROP').catch(()=>{});
    await execPromise('iptables -I FORWARD -p udp --dport 4500 -j DROP').catch(()=>{});
    await execPromise('iptables -I FORWARD -p udp --dport 1701 -j DROP').catch(()=>{});

    // Block OpenVPN (Common ports: UDP 1194, TCP 1194)
    await execPromise('iptables -I FORWARD -p udp --dport 1194 -j DROP').catch(()=>{});
    await execPromise('iptables -I FORWARD -p tcp --dport 1194 -j DROP').catch(()=>{});

    // Block WireGuard (UDP 51820)
    await execPromise('iptables -I FORWARD -p udp --dport 51820 -j DROP').catch(()=>{});

    // Block DNS over TLS (TCP 853)
    await execPromise('iptables -I FORWARD -p tcp --dport 853 -j DROP').catch(()=>{});
    
    console.log('[NET] Anti-VPN Rules Applied.');
  } catch (e) {
    console.error('[NET] Error applying Anti-VPN rules:', e.message);
  }
}

async function removeAntiVPNRules() {
  console.log('[NET] Removing Anti-VPN Rules...');
  try {
    await execPromise('iptables -D FORWARD -p tcp --dport 1723 -j DROP').catch(()=>{});
    await execPromise('iptables -D FORWARD -p 47 -j DROP').catch(()=>{});
    await execPromise('iptables -D FORWARD -p udp --dport 500 -j DROP').catch(()=>{});
    await execPromise('iptables -D FORWARD -p udp --dport 4500 -j DROP').catch(()=>{});
    await execPromise('iptables -D FORWARD -p udp --dport 1701 -j DROP').catch(()=>{});
    await execPromise('iptables -D FORWARD -p udp --dport 1194 -j DROP').catch(()=>{});
    await execPromise('iptables -D FORWARD -p tcp --dport 1194 -j DROP').catch(()=>{});
    await execPromise('iptables -D FORWARD -p udp --dport 51820 -j DROP').catch(()=>{});
    await execPromise('iptables -D FORWARD -p tcp --dport 853 -j DROP').catch(()=>{});
    console.log('[NET] Anti-VPN Rules Removed.');
  } catch (e) {
     console.error('[NET] Error removing Anti-VPN rules:', e.message);
  }
}

async function initAntiVPN() {
  try {
    const row = await db.get("SELECT value FROM config WHERE key = 'anti_vpn_enabled'");
    if (row && row.value === '1') {
        await applyAntiVPNRules();
    }
  } catch (e) {
    console.error('[NET] Init Anti-VPN Error:', e.message);
  }
}

async function toggleAntiVPN(enabled) {
  try {
    await db.run("INSERT OR REPLACE INTO config (key, value) VALUES ('anti_vpn_enabled', ?)", [enabled ? '1' : '0']);
    if (enabled) {
      await applyAntiVPNRules();
    } else {
      await removeAntiVPNRules();
    }
    return true;
  } catch (e) {
    console.error('[NET] Toggle Anti-VPN Error:', e.message);
    throw e;
  }
}
