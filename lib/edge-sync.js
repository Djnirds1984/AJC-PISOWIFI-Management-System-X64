...
class EdgeSync {
   ...
   */
  async recordSale(saleData, shouldQueue = true) {
    if (!this.supabase || !this.machineId) {
      // Queue sale if offline or not linked
      if (shouldQueue) this.queueSync('sale', saleData);
      return false;
    }
    ...
    try {
      ...
      if (error) {
        // Check for trigger error related to vendor_dashboard_realtime (Foreign Key Violation)
        if (error.message && (error.message.includes('vendor_dashboard_realtime') || error.message.includes('foreign key constraint'))) {
            console.warn(`[EdgeSync] Critical: Vendor ID ${vendorId} caused DB constraint violation. Resetting local identity and clearing cloud association.`);
            
            // 1. Reset local identity
            this.vendorId = null;
            await this.saveLocalVendorId(null);

            // 2. Clear cloud association (set vendor_id to null in vendors table)
            // This prevents re-fetching the bad ID immediately
            try {
                await this.supabase
                    .from('vendors')
                    .update({ vendor_id: null })
                    .eq('id', this.machineId);
                console.log('[EdgeSync] Successfully cleared invalid vendor_id from cloud.');
            } catch (cleanupError) {
                console.error('[EdgeSync] Failed to clear invalid vendor_id from cloud:', cleanupError);
            }
        }
        throw error;
      }
  ...
  async processQueue() {
    ...
    // Optimization: Check if we have a vendor ID before trying to process sales.
    // If we are not claimed by a vendor, we cannot process sales.
    if (!this.vendorId) {
        // Try to fetch identity one last time before giving up on this batch
        await this.registerOrFetchMachine();
        
        // If still no vendor ID, and we have sales in the queue, pause processing
        if (!this.vendorId) {
             const hasSales = this.queue.some(item => item.type === 'sale');
             if (hasSales) {
                 // Log only if we haven't logged recently (simple debounce could be added, but for now just log once per batch attempt)
                 console.log(`[EdgeSync] Pausing queue processing (${this.queue.length} items) - Waiting for vendor claim.`);
                 return;
             }
        }
    }
    ...
    for (const item of itemsToProcess) {
      ...
      try {
        if (item.type === 'sale') {
            success = await this.recordSale(item.data, false);
        }
      } catch (e) { /* ignore */ }
